...
int main(int argc, char** argv) {
    auto lines = getLines(false); // false = all lines, not just horizontal
    ...
    for (auto line : lines) {
        ...
        if (xEqual) {
            ...
            int xDir = ((x1 < x2) * 2) - 1;
            int yDir = ((y1 < y2) * 2) - 1;
            for (int i = 0; i <= (xMax - xMin); i++) { // xMax-xMin = yMax-yMin (45 degrees), could use either.
                diagram[y1+(i*yDir)][x1+(i*xDir)] += 1;
            }
        }
    }
